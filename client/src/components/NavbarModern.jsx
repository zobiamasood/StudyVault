  ...
  return (
    <header className="sticky top-0 z-30 border-b border-white/10 bg-deep-teal text-white shadow-lg shadow-deep-teal/10">
      <div className="mx-auto flex min-h-18 max-w-7xl items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
        <NavLink to={user ? '/' : '/login'} className="group flex items-center gap-3" onClick={() => setMenuOpen(false)}>
        ...
        <nav className={`${menuOpen ? 'flex' : 'hidden'} absolute left-4 right-4 top-20 flex-col gap-2 rounded-2xl border border-deep-teal/10 bg-white p-3 text-deep-teal shadow-xl md:static md:flex md:flex-row md:items-center md:gap-1 md:border-0 md:bg-transparent md:p-0 md:shadow-none`} aria-label="Main navigation">
          {user && navItems.map((item) => <NavLink key={item.path} to={item.path} onClick={() => setMenuOpen(false)} className={({ isActive }) => `flex items-center gap-2 rounded-lg px-3 py-2 text-sm font-medium transition md:text-white/75 ${isActive ? 'bg-muted-gold/15 text-deep-teal md:bg-white/10 md:text-white' : 'hover:bg-deep-teal/5 hover:text-deep-teal md:hover:bg-white/10 md:hover:text-white'}`}>{item.label === 'Add Resource' && <Plus size={16} />}{item.label === 'Resources' && <Search size={16} />}{item.label}</NavLink>)}
